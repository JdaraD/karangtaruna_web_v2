...
new class extends Component
{
    ...
    {
        $this->runningText = runningText::latest()
            ->take(3)
            ->get();
    }
            ...
            <div class="flex justify-center items-center h-8 bg-[#9CB080] border-t border-white/10 px-4">
                <div class="flex items-center gap-3 w-full max-w-7xl">
                    <div class="flex items-center gap-2 shrink-0">
                        ...
                        <div class="h-4 w-0.5 bg-white/40"></div>
                    </div>

                    <div class="overflow-hidden w-full text-white whitespace-nowrap">
                        <div class="flex gap-2 animate-marquee tracking-wide">
                            @foreach ($runningText as $text )
                                <p class="text-xs md:text-sm font-medium text-white capitalize">
                                    {{ $text->text }}@if (!$loop->last) |@endif
                                </p>
                            @endforeach
                        </div>
                    </div>
