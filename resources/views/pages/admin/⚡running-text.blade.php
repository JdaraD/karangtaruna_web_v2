...
new class extends Component
{
    ...
    public function btnOpenEdit($id)
    {
        ...
        $this->runningTextId = $runningText->id;
        $this->judul = $runningText->judul;
        $this->text = $runningText->text;
    ...
    public function update()
    {
        ...
        try {
            $runningText = RunningText::findOrFail(
                $this->runningTextId
            );
            $runningText->update([
                'text' => $this->text,
            ...
            $this->editSuccess = 'Data Gagal Diedit!';
        }
    }
...
<section class="flex flex-col gap-4 w-full shrink-0 3xl:h-210 lg:h-157.5 h-full overflow-y-auto scrollbar-none">
                    ...
                    <div class="flex flex-col w-full h-38 gap-2 p-2 bg-[#9CB080] rounded-md shadow-md hover:scale-102 duration-120 ease-in-out transition-transform">
                        ...
                        <div class="flex w-full h-8 gap-1 p-1 justify-between items-center bg-[#618764]/40 rounded-md">
                            ...
                            <div class="flex gap-1">
                                <button type="button" wire:click="btnOpenEdit({{ $text->id }})" class="flex bg-yellow-500 hover:bg-yellow-700 justify-center items-center w-6 h-6 rounded-md shadow-md cursor-pointer" title="Lihat">
    ...
    @if ($overlayEdit)
        <article class="absolute flex top-0 left-0 items-center justify-center w-full h-full bg-gray-400/60 z-50">
            <form wire:submit.prevent="update" class="flex flex-col w-fit h-fit gap-4 p-4 bg-white rounded-md">
                
                <div class="flex w-full h-fit gap-1 justify-between items-center bg-gray-100 rounded-md p-2">
                    <div class="flex w-full h-auto gap-1 items-center">
                        <h1 class="font-semibold text-base text-black capitalize">Edit Running Text</h1>
                    </div>
                    <div class="flex w-[30%] h-auto gap-1 justify-end items-center">
                ...
                <div class="flex flex-col w-full gap-5 pt-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 items-center gap-2">
                        ...
                        <input type="text" wire:model="judul" name="judul" id="judul" placeholder="Masukkan judul berita" class="md:col-span-3 w-full rounded-md text-black border border-gray-300 bg-gray-100 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200">
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 items-start gap-2">
                        ...
                        <textarea name="text" wire:model="text" id="text" rows="7" placeholder="Masukkan isi berita" class="md:col-span-3 w-full resize-none rounded-md text-black border border-gray-300 bg-gray-100 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"></textarea>
                    </div>
