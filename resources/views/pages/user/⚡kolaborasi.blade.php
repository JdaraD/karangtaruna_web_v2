...
new class extends Component
{
    ...
    {
        $this->wilayah = wilayahKolaborasi::all();
    }
        ...
        <div class="flex flex-wrap w-full h-full justify-center items-center lg:gap-4 md:gap-4 gap-2">
            @foreach ($wilayah as $wh)
                ...
                @php
                    ...
                    if ($wh->coverFoto && $wh->coverFoto->image) {
                        $images = json_decode($wh->coverFoto->image, true);
                ...
                <a href="{{ route('detail-kolaborasi', $wh->id) }}" class="flex relative flex-col lg:w-82 md:w-80 w-30 lg:h-60 md:h-58 h-28 bg-gray-300 rounded-lg shadow-md hover:scale-102 transition-transform duration-120 ease-in-out">
                    ...
                    <div class="flex absolute bottom-0 w-full lg:h-10 md:h-10 h-6 justify-center items-center bg-gray-400/70 rounded-b-lg">
                        ...
                        <p class="text-black font-[poppins] lg:text-base md:text-sm text-xs font-semibold">
                            {{ $wh->nama_wilayah }}
                        </p>
